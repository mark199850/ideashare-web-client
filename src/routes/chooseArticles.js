    ...
    const handleShowNotFound = () => setShowNotFound(true);
      ...
      useEffect(() => {
          Axios.get('https://nodejs-server-test-app.herokuapp.com/api/login/user').then((response) => {
            ...
            Axios.get('https://nodejs-server-test-app.herokuapp.com/api/get/article/allById', {
          headers: {
    ...
    return(
        <div>
          {ArticleNameList.map((val) => {
            ...
            <Modal show={showNotFound} onHide={handleCloseNotFound}>
                <Modal.Header closeButton>
                    <Modal.Title>Oops</Modal.Title>
                </Modal.Header>
                <Modal.Body>It seems you do not have any articles</Modal.Body>
                <Modal.Footer>
                <Button className="lets-go-btn" variant="dark" onClick={() => {handleCloseNotFound(); routeChangeCreateArticle();}}>
                    Let's go and create one <span className="lets-go-arrow">&#10140;</span>
                </Button>
                </Modal.Footer>
            </Modal>
        </div>
