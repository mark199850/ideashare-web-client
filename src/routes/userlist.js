...
import Axios from 'axios';
 ...
 const [record,setRecord] = useState([]);
  ...
  Axios.get('https://nodejs-server-test-app.herokuapp.com/api/get/user').then((response) => {
  ...
  const deleteUser = (userId) =>{
    Axios.delete(`https://nodejs-server-test-app.herokuapp.com/api/delete/user/${userId}`).then(() => {  // with altgr+7 you can add variables to it
    refreshUserData();
    });
  };
    ...
    const searchRecords = () =>
    {
        Axios.get(`https://nodejs-server-test-app.herokuapp.com/api/get/user/search/${search}`).then(response => {
